...
pub fn claim_prize(ctx: Context<ClaimPrize>) -> Result<()> {
    ...
    require!(
        game.prize_pool > 0,
        RummikubError::PrizeAlreadyClaimed
    );

    // Calculate house fee (5%)
    let house_fee = (game.prize_pool as u128)
        .checked_mul(HOUSE_FEE_BPS as u128)
        .unwrap()
        .checked_div(10000)
        .unwrap() as u64;
    
    let winner_prize = game.prize_pool.checked_sub(house_fee).unwrap();

    // Transfer house fee to treasury
    **ctx.accounts.game.to_account_info().try_borrow_mut_lamports()? -= house_fee;
    **ctx.accounts.treasury.try_borrow_mut_lamports()? += house_fee;

    // Transfer winner prize to winner
    **ctx.accounts.game.to_account_info().try_borrow_mut_lamports()? -= winner_prize;
